...
.PHONY: all lazy_install mix_install lazy_install_kube agent_install clean debugger_install image_remoted lazy_install_remoted gen_pb
...
runner_image:
	...
	sudo docker buildx build --platform linux/amd64 \
		...
		-f ./image/Dockerfile.$(repo_image) --build-arg SHR_C_T=runner --build-arg RUNNER_TOKEN=$(TF_VAR_RUNNER_TOKEN) \
		--build-arg MNS_URL=$(TF_VAR_MNS_URL_PUBLIC) \
		--build-arg IMAGE_RETRIEVE_USERNAME=$(TF_VAR_IMAGE_RETRIEVE_USERNAME)  \
		--build-arg IMAGE_RETRIEVE_PWD=$(TF_VAR_IMAGE_RETRIEVE_PWD) --build-arg IMAGE_RETRIEVE_SERVER=$(TF_VAR_IMAGE_RETRIEVE_SERVER) \
		--build-arg GO_ARCH=amd64 --build-arg SLS_ENC_KEY="$(SLS_ENC_KEY)" --build-arg SLS_TF_CTL="$(SLS_TF_CTL)" \
		--build-arg CTX_USERNAME=$(CTX_USERNAME) --build-arg CTX_PWD=$(CTX_PWD) --build-arg CTX_PWD=$(CTX_PWD) \
...
dipatcher_image:
	sudo docker buildx build --platform linux/amd64 \
		...
		--build-arg SLS_ENC_KEY="$(SLS_ENC_KEY)" --build-arg SLS_TF_CTL="$(SLS_TF_CTL)" \
		--build-arg GIT_ACCESS_TOKEN_USR=$(GIT_ACCESS_TOKEN_USR) --build-arg GIT_ACCESS_TOKEN_PWD=$(GIT_ACCESS_TOKEN_PWD) \
